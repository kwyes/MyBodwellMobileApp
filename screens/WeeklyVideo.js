...
export default class WeeklyVideo extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      media: [],
  ...
  async componentDidMount() {
    var cantoFldr = await AsyncStorage.getItem("Canto-WeeklyVideo");
  ...
  redirectStudentVideoDetail = rowData => {
    const { navigation } = this.props;
    if (rowData) {
      navigation.navigate("weeklyVideoDetail", {
        param: rowData
      });
    } else {
        ...
        >
          <Block flex row style={{ flexWrap: "wrap" }}>
            {media.map((rowData, index) =>
              <TouchableOpacity
                onPress={() => this.redirectStudentVideoDetail(rowData)}
                style={styles.image}
                key={index}
              >
                <Image
                ...
                <Block flex style={styles.containerAbs}>
                  <Block style={styles.durationContainer}>
                    <Text color={"white"} size={RFValue(10)} style={{ opacity: 1 }}>
